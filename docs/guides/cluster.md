...
- `src/config.rs`：`ClusterConfig` 和 Agent 模板；`service.rs`：审批 owner 与路由。
- `src/prompt.rs`/`tools/mod.rs`：集群契约、spawn schema 和子工具过滤。
- 消费端展示：TUI 消费端仓库在其 `src/app.rs`/`src/projection.rs` 只消费 `ChildSessionProgress` 与 `child_status`，不维护调度/审批 owner/批次状态机。
