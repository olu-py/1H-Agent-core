...
- 下游接入：`src/agent.rs`（`WRITE_TOOLS`/角色 infer）、`prompt.rs`；消费端展示映射：消费端仓库的 `src/ui.rs`（`tool_display_name`/`tool_compact_summary`/`tool_risk`/`argument_label`）与 protocol 的 `ToolCall`/`ToolStarted`/`ToolFinished` 事件。
...
- 迭代过滤器按改动选择：`tools::filesystem::tests`、`tools::tests`（会话放行 policy）、`security::tests`、`tools::process::tests`；UI 翻译改动加 `ui::tests`。
