...
- core `main` 的 required checks：`Linux quality`、`Minimum Rust (1.85.0)`、`Test (macos-latest)`、`Test (windows-latest)`；跨仓库升级 PR 不直接写 `main`。兼容性方向单一：core CI 只验证 core 自身，不 checkout 下游 `main` 做 path-patch 编译（下游未适配时必然伪失败）；下游对上游的兼容由消费端 `core-upgrade.yml` 负责。
